...
/**
  ...
  */
object TruthMatrix extends ExperimentsCommonConfig {
  def main(args: Array[String]): Unit = {
    ...
    SparkLOAN.withSparkSession("WAHRHEITSMATRIX2") {
      session => {
        ...
        val nxor = udf {
          (label: String, tool: String) => if (label.equals(tool)) "1" else "0"
        }
        ...
        // val columns: Seq[(String, Column)] = tools.map(t => (t, truthDF(t)))

        import session.implicits._
        val transposedDF: DataFrame = tools.map(columnName => {
          val columnForTool = truthDF.select(columnName)
          val toolsVals: Array[Double] = columnForTool
            .rdd
            .map(element => element.getString(0).toDouble)
            .collect()
          val valsVector = org.apache.spark.mllib.linalg.Vectors.dense(toolsVals)
          (columnName, valsVector)
        }).toDF("tool-name", "features")

        val toolNameIndexer = new StringIndexer()
          .setInputCol("tool-name")
          .setOutputCol("tool-id")

        val matrixWithToolsId = toolNameIndexer.fit(transposedDF).transform(transposedDF)
        matrixWithToolsId.show()

        val labeledData: RDD[LabeledPoint] = matrixWithToolsId
          .select("tool-id", "features")
          .map(row => {
            val label = row.getDouble(0)
            val features = row.getAs[org.apache.spark.mllib.linalg.Vector](1)
            LabeledPoint(label, features)
          }).rdd

       // MLUtils.saveAsLibSVMFile(labeledData, "/Users/visenger/research/datasets/clustering")
