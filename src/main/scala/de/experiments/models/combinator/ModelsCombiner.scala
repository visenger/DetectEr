...
object ModelsCombinerStrategy extends ExperimentsCommonConfig {
    ...
    SparkLOAN.withSparkSession("MODELS-COMBINER") {
      session => {
        process_data { data => {
          ...
          evalStacking.printResult(s"STACKING FOR TOOLS ONLY ON $dataSetName ")
        }
        }
      }
    }
      ...
      session => {
        ...
        process_data {
          data => {
            ...
            val testLabPointsDF: DataFrame = FormatUtil.prepareToolsDataToLabPointsDF(session, testDF, features)
            ...
            majorityEval.printResult(s"MAJORITY WINS (TOOLS) FOR $dataSetName")
  ...
  //            .setSeed(1234L)
  //            .setMaxIter(100)
  //          val nnTrain: DataFrame = FormatUtil.prepareDataToLIBSVM(session, trainDF, FullResult.tools)
